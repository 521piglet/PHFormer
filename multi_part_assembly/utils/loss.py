...
def corr_matrix_loss(pred, gt, valids):
    B, P = valids.shape
    idx = torch.arange(0, P, device=valids.device).long()
    # gt[:, idx, idx] = 1.
    loss = (pred - gt).pow(2)
    valid_mask = valids.unsqueeze(-1) * valids.unsqueeze(1)
    valid_mask[:, idx, idx] = 0.
    valid_mask = valid_mask.detach()
    loss_per_data = (loss * valid_mask).sum(-1).sum(-1) / valid_mask.sum(-1).sum(-1)
    return loss_per_data
def corr_BCE_loss(pred, gt, valids):
    B, P = valids.shape
    idx = torch.arange(0, P, device=valids.device).long()
    valid_mask = valids.unsqueeze(-1) * valids.unsqueeze(1)
    valid_mask[:, idx, idx] = 0.
    ...
    loss = - gt * pred.log() - (1 - gt) * (1 - pred).log()

    # n_p = (gt * valid_mask).sum([1, 2], keepdim=True)
    # n_n = ((1 - gt) * valid_mask).sum([1, 2], keepdim=True)
    # loss = - (n_n + 1) / n_p * gt * pred.log() - (1 - gt) * (1-pred).log()
...
def rot_l2_loss(rot1, rot2, valids):
    ...
    loss_per_data = _valid_mean(loss_per_data, valids)
    return loss_per_data
def rot_l1_loss(rot1, rot2, valids):
    """L2 loss for rotation.
    ...
    quat1, quat2 = rot1.rot, rot2.rot
    # since quat == -quat
    rot_l1_1 = (quat1 - quat2).abs().sum(dim=-1)  # [B, P]
    # rot_l1_2 = (quat1 + quat2).abs().sum(dim=-1)  # [B, P]
    # loss_per_data = torch.minimum(rot_l1_1, rot_l1_2)
    # loss_per_data = _valid_mean(loss_per_data, valids)
    loss_per_data = _valid_mean(rot_l1_1, valids)
    return loss_per_data
...
def reconstruction_cd_loss(pts1, pts2):
    """
    Args:
        pts1: [B M 3]
        pts2: [B N 3]
...
def stacked_shape_cd_loss(
    ...
    Args:
        pts: [B , N, 3], model input point cloud to be transformed
        trans1: [B,  3]
        trans2: [B, P, 3]
        rot1: [B,  4/(3, 3)], Rotation3D, quat or rmat
        rot2: [B, 4/(3, 3)], Rotation3D, quat or rmat
        valids: [B, P], 1 for input parts, 0 for padded parts
    ...
    stacked_points = pts[valid_mask]

    pts1 = transform_pc(trans1[valid_mask], rot1[valid_mask], stacked_points)
    pts2 = transform_pc(trans2[valid_mask], rot2[valid_mask], stacked_points)

    dist1, dist2 = chamfer_distance(pts1, pts2)  # [B, P*N]
    dist = torch.mean(dist1, dim=-1) + torch.mean(dist2, dim=-1)
    pad_dist = torch.zeros_like(valids)
    pad_dist[valid_mask] = dist
    if training:
        loss_per_data = pad_dist.mean(-1)
    # this is the correct SCD calculation
    else:
        loss_per_data = pad_dist.sum(-1) / valids.sum(-1)
...
def shape_cd_loss(
    ...
    valids = valids.float().detach()
    if training:
        shape1 = pts1.flatten(0, 1)  # [B, P*N, 3]
        shape2 = pts2.flatten(0, 1)
        dist1, dist2 = chamfer_distance(shape1, shape2)  # [B, P*N]

        valids = valids.unsqueeze(2).repeat(1, 1, N).view(-1, N)
        dist1 = dist1 * valids
        dist2 = dist2 * valids
        # we divide the loss by a fixed number `P`
        # this is actually an automatic hard negative loss weighting mechanism
