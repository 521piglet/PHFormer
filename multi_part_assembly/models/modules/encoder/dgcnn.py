...
def get_graph_feature(x, knn_idx =None, k=20):
    """x: [B, C, N]"""
    if knn_idx is None:
    ...
    return feature, knn_idx      # (batch_size, 2*num_dims, num_points, k)
...
class DGCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        x, knn_idx = get_graph_feature(x)   # (batch_size, 3, num_points) -> (batch_size, 3*2, num_points, k)
        x = self.conv1(x)          # (batch_size, 3*2, num_points, k) -> (batch_size, 64, num_points, k)
        x1 = x.max(dim=-1)[0]      # (batch_size, 64, num_points, k) -> (batch_size, 64, num_points)

        x, _ = get_graph_feature(x1, knn_idx)  # (batch_size, 64, num_points) -> (batch_size, 64*2, num_points, k)
        x = self.conv2(x)          # (batch_size, 64*2, num_points, k) -> (batch_size, 64, num_points, k)
        x2 = x.max(dim=-1)[0]      # (batch_size, 64, num_points, k) -> (batch_size, 64, num_points)

        x, _  = get_graph_feature(x2, knn_idx)  # (batch_size, 64, num_points) -> (batch_size, 64*2, num_points, k)
        x = self.conv3(x)          # (batch_size, 64*2, num_points, k) -> (batch_size, 128, num_points, k)
        x3 = x.max(dim=-1)[0]      # (batch_size, 128, num_points, k) -> (batch_size, 128, num_points)
        x , _ = get_graph_feature(x3, knn_idx)  # (batch_size, 128, num_points) -> (batch_size, 128*2, num_points, k)
