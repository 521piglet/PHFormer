...
from multi_part_assembly.utils import trans_l2_loss, rot_points_cd_loss, reconstruction_cd_loss,\
    shape_cd_loss, rot_cosine_loss, rot_l1_loss, rot_l2_loss, rot_points_l2_loss, chamfer_distance
...
class BaseModel(pl.LightningModule):
    ...
    def _setup(self):
        ...
        # self.pose_feat_dim = self.cfg.model.pose_feat_dim
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log_dict(avg_loss, sync_dist=True, rank_zero_only=True)
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.save_path is not None:
            with open(os.path.join(self.save_path, 'ablation_B.pk'), 'wb') as f:
                pickle.dump(self.stats, f)


    def squeeze_length(self, data_dict):
        new_data_dict = {}
        valids = data_dict['part_valids']
        P = valids.sum(-1).max().int()
        B = valids.shape[0]
        for key, value in data_dict.items():
            if key in ['part_pcs', 'part_quat', 'part_trans', 'part_valids',
                       'instance_label', 'part_label', 'part_ids']:
                new_data_dict[key] = value[:, :P]
            elif key == 'adj_labels':
                new_data_dict[key] = value[:, :P, :P]
            elif key == 'gt_rel_pose':
                new_data_dict[key] = value[:, :P, :P].reshape(B, P*P, -1)
            elif key in ['merge_pc', 'data_id']:
                new_data_dict[key] = value
            else:
                Exception(f'Wrong key value: {key}')
    ...
    def forward_pass(self, data_dict, mode, optimizer_idx):
        ...
        data_dict = self.squeeze_length(data_dict)
    ...
    def _calc_loss(self, out_dict, data_dict):
        ...
        if self.cfg.loss.use_rot_loss:
            ...
            # loss_dict['rot_loss'] = rot_l1_loss(pred_rot, new_rot, valids)
    ...
    def cal_reconstruction_loss(self, pred_merge_pc, merge_pc,):
        return reconstruction_cd_loss(pred_merge_pc, merge_pc)
    ...
    def _calc_metrics(self, data_dict, out_dict, gt_trans, gt_rot):
        ...
        if not self.semantic:
            for metric in ['mse', 'rmse', 'mae']:
                metric_dict[f'trans_{metric}'] = trans_met = trans_metrics(
                    pred_trans, gt_trans, valids, metric=metric)
                metric_dict[f'rot_{metric}'] = rot_met = rot_metrics(
            ...
            if self.stats is not None:
                self.stats['part_acc'].append(metric_dict['part_acc'].cpu().numpy())
                ...
                self.stats['part_valids'].append(valids.cpu().numpy())
        return metric_dict
